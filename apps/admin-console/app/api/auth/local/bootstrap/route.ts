...
export async function POST(request: NextRequest) {
  ...
  if (!payload) return NextResponse.json({ error: { message: "注册信息不完整", code: "validation_error" } }, { status: 400 });
  try {
    const upstream = await localAuthRequest("bootstrap", payload); const body = await upstream.json() as { accessToken?: string; owner?: unknown; error?: unknown };
    if (!upstream.ok || !body.accessToken) return NextResponse.json({ error: body.error ?? { message: "主组织账号创建失败", code: "authentication_error" } }, { status: upstream.status });
    const created = await createSession(body.accessToken);
    const response = NextResponse.json({ authenticated: true, csrfToken: created.session.csrfToken, owner: body.owner }, { status: 201 });
